...
function TemplateMessages(props) {
    ...
    const bulkSend = (template) => {
        const payload = generateTemplateMessagePayload(
    ...
    useEffect(() => {
        const onSendTemplateMessageError = function (msg, data) {
            ...
            if (dialogContent.current) {
                dialogContent.current.scrollTop =
                    dialogContent.current.scrollHeight;
            }
        };
        ...
        const onSentTemplateMessage = function (msg, data) {
            ...
            if (
                ...
                JSON.stringify(data) ===
                    JSON.stringify(
                        generateTemplateMessagePayload(sentTemplateMessage)
                    )
            ) {
                hideDialog();
                setSending(false);
    ...
    return (
        <div className="templateMessagesOuter">
            ...
            {props.isLoadingTemplates ? (
                ...
                <div
                    className="templateMessages"
                    data-test-id="template-messages"
                >
                    ...
                    {Object.entries(templates).map((template, index) => (
                        <div
                            key={template[0]}
                            className="templateMessageWrapper"
                        >
