...
export const getDisplayAssignmentAndTaggingHistory = () => {
    const result = getStorage().getItem(STORAGE_TAG_DISPLAY_ASSIGNMENT_AND_TAGGING_HISTORY);
    return result ? result === 'true' : true;
}

export const setDisplayAssignmentAndTaggingHistory = (willDisplay) => {
    return getStorage().setItem(STORAGE_TAG_DISPLAY_ASSIGNMENT_AND_TAGGING_HISTORY, willDisplay);
}
...
export const storeContactProvidersData = (data) => {
    getStorage().setItem(STORAGE_TAG_CONTACT_PROVIDERS_DATA, JSON.stringify(data));
}
